...
- **TorchScript Scripting**:
  ...
  **Code example**:
  A function to save a torch model or function a scripted module is quite simple:
  ```
  ...
  ```
   ...
   def simple_function(x, y):
    ...
    return r
  ```
  For a  simple function the process is simple and just saves the model as a .pt model:
...
Luckily, do you not have to stick with only one method. 

- **Which method to use as the default**:
