...
- **The difference between torch.jit.trace and torch.jit.script**:
  ...
  - **TorchScript Scripting**:
    ...
    **What it does**: In simple terms, scripting takes the pytorch model and transofms it into a lower level representation that can be used outside of Python. The scripting process aims to optimize the model's performance. It can remove Python-specific overhead, improve memory usage, and enable the use of Just-In-Time (JIT) compilation techniques for faster execution.\
    ...
    **Method specific problems**:
      ...
      + Capture device, for example, if we had cpu during tracing, it will be saved as a cpu specific device 
