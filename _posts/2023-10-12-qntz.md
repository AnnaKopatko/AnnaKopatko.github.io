...
```
And a more complicated newral network with a ReLu activation function and a decision gate:
...
- **Code example**:
  ...
  But with a more complicated model there is an error:
  
  ```
  if __name__ == '__main__':
    DecisionModel = DecisionConvNet()
    transform_model_to_torch_script(DecisionModel)
  ```
  You can also see that the error is quite clear and easy to understand:
  ...
  ```
  The ReLu function is not implemented in scripting and this is an example of a very common error of a limited torch.script functuonality. 
...
- First, lets use **tracing as a default method**, and only script the decision gate. For that, just change the moddel itself, the same tracing function can be used:
...
- Now, lets use **scripting as a default method**, and only trace the ReLu part. For that, just change the moddel itself, the same tracing function can be used:
